...
def _compute_ddqn_loss_same_objectives(agent, target_agent, batch, cfg):
    ...
    device = cfg.device
    timing = {
        "collate_pad_time": 0.0,
        "to_cuda_time": 0.0,
        "forward_current_time": 0.0,
        "forward_next_online_time": 0.0,
        "forward_next_target_time": 0.0,
        "loss_time": 0.0,
    }

    collate_start = time.perf_counter()
    archive_mask_np = build_row_mask(x_true)
    candidate_mask_np = build_row_mask(x_sur)
    next_archive_mask_np = build_row_mask(next_x_true)
    next_candidate_mask_np = build_row_mask(next_x_sur)
    ...
    timing["collate_pad_time"] = time.perf_counter() - collate_start

    to_cuda_start = time.perf_counter()
    archive_mask = torch.as_tensor(archive_mask_np, dtype=torch.bool, device=device)
    candidate_mask = torch.as_tensor(candidate_mask_np, dtype=torch.bool, device=device)
    next_archive_mask = torch.as_tensor(next_archive_mask_np, dtype=torch.bool, device=device)
    next_candidate_mask = torch.as_tensor(next_candidate_mask_np, dtype=torch.bool, device=device)
    ...
    actions = torch.tensor(actions_arr, dtype=torch.long, device=device)
    ...
    sync_device(device)
    timing["to_cuda_time"] = time.perf_counter() - to_cuda_start

    forward_current_start = time.perf_counter()
    out = agent(
        x_true=x_true,
        y_true=y_true,
        x_sur=x_sur,
    ...
    sync_device(device)
    timing["forward_current_time"] = time.perf_counter() - forward_current_start

    q_values = out["q_values"]
    q_sa = q_values.gather(1, actions.view(-1, 1)).squeeze(1)

    with torch.no_grad():
        forward_next_online_start = time.perf_counter()
        next_online = agent(
            x_true=next_x_true,
            y_true=next_y_true,
            x_sur=next_x_sur,
        ...
        sync_device(device)
        timing["forward_next_online_time"] = time.perf_counter() - forward_next_online_start

        next_actions = torch.argmax(next_online["q_values"], dim=1)

        forward_next_target_start = time.perf_counter()
        ...
        sync_device(device)
        timing["forward_next_target_time"] = time.perf_counter() - forward_next_target_start
    ...
    sync_device(device)
    timing["loss_time"] = time.perf_counter() - loss_start
    metrics = {
    ...
    return loss, metrics, len(x_true), timing
...
def compute_ddqn_loss(agent, target_agent, batch, cfg):
    ...
    split_group_start = time.perf_counter()
    for n_obj, indices in groups.items():
        subbatch = []
        for item in batch_items:
            subbatch.append([item[i] for i in indices])
        timing["split_group_time"] += time.perf_counter() - split_group_start

        group_loss, group_metrics, group_count, group_timing = _compute_ddqn_loss_same_objectives(
            agent=agent,
        ...
        for key in timing:
            if key == "split_group_time":
                continue
            timing[key] += float(group_timing[key])
    ...
    return weighted_loss, metrics, timing
...
def _rollout_episode_impl(state_dict_cpu, cfg_dict, problem_name, dim, seed, epsilon):
    ...
    return {
        ...
        "encoder_forward_time": float(encoder_forward_time),
    }
...
def train_disc_ddqn_ray(
    ...
    for it in range(cfg.train_iters):
        ...
        for result in results:
            ...
            ep_encoder_forward_time = float(result.get("encoder_forward_time", 0.0))
            bucket["encoder_forward_time"].append(ep_encoder_forward_time)
            bucket["encoder_forward_time_per_fe"].append(ep_encoder_forward_time / float(ep_steps))
        per_env_summaries = {}
        for key, stats in sorted(per_env_stats.items()):
            ...
            per_env_summaries[key] = {
                ...
                "encoder_forward_time": float(np.mean(stats["encoder_forward_time"])),
                "encoder_forward_time_per_fe": float(np.mean(stats["encoder_forward_time_per_fe"])),
            }
        mean_ep_reward = (
        ...
        mean_rollout_encoder_forward_time = (
            float(np.mean([v["encoder_forward_time"] for v in per_env_summaries.values()]))
            if per_env_summaries
            else 0.0
        )
        mean_rollout_encoder_forward_time_per_fe = (
            float(np.mean([v["encoder_forward_time_per_fe"] for v in per_env_summaries.values()]))
            if per_env_summaries
            else 0.0
        )
        for key, stats in per_env_summaries.items():
            log(
                ...
                f"encoder_forward_time = {stats['encoder_forward_time']:.6f}, "
                f"encoder_forward_time/FE = {stats['encoder_forward_time_per_fe']:.6f}"
            )
        ...
        if len(replay) < cfg.batch_size:
            ...
            log(
                ...
                f"envs_active={len(env_specs)}/{len(env_specs)} | "
                f"replay={len(replay)} | skip update | "
                f"rollout_encoder_forward_time={mean_rollout_encoder_forward_time:.6f} | "
                f"rollout_encoder_forward_time_per_fe={mean_rollout_encoder_forward_time_per_fe:.6f} | "
        ...
        for update_idx in range(int(cfg.updates_per_epoch)):
            ...
            sample_batch_time = time.perf_counter() - sample_batch_start
            loss, ddqn_metrics, ddqn_timing = compute_ddqn_loss(agent, target_agent, batch, cfg)
            ...
            update_metrics = {
                ...
                "sample_batch_time": float(sample_batch_time),
                "split_group_time": float(ddqn_timing["split_group_time"]),
                "collate_pad_time": float(ddqn_timing["collate_pad_time"]),
                "to_cuda_time": float(ddqn_timing["to_cuda_time"]),
                "forward_current_time": float(ddqn_timing["forward_current_time"]),
                "forward_next_online_time": float(ddqn_timing["forward_next_online_time"]),
                "forward_next_target_time": float(ddqn_timing["forward_next_target_time"]),
                "loss_time": float(ddqn_timing["loss_time"]),
                "backward_time": float(backward_time),
                "optimizer_step_time": float(optimizer_step_time),
            }
        ...
        for key in [
            ...
            "collate_pad_time",
            "to_cuda_time",
            "forward_current_time",
            "forward_next_online_time",
            "forward_next_target_time",
            "loss_time",
            "backward_time",
            "optimizer_step_time",
        ]:
        ...
        log(
            ...
            f"sample_batch_time={mean_update_metrics['sample_batch_time']:.6f} | "
            f"split_group_time={mean_update_metrics['split_group_time']:.6f} | "
            f"collate_pad_time={mean_update_metrics['collate_pad_time']:.6f} | "
            f"to_cuda_time={mean_update_metrics['to_cuda_time']:.6f} | "
            f"forward_current_time={mean_update_metrics['forward_current_time']:.6f} | "
            f"forward_next_online_time={mean_update_metrics['forward_next_online_time']:.6f} | "
            f"forward_next_target_time={mean_update_metrics['forward_next_target_time']:.6f} | "
            f"loss_time={mean_update_metrics['loss_time']:.6f} | "
            f"backward_time={mean_update_metrics['backward_time']:.6f} | "
            f"optimizer_step_time={mean_update_metrics['optimizer_step_time']:.6f} | "
